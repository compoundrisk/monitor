...
aggregate_dimension <- function(
  ...
  sheet <- sheet %>% 
    # Select only the normalised variables that are actually used in the monitor
    # (as defined in `indicators-list.csv`)
    select(Country,
  ...
  sheet <- sheet %>% mutate(
    ...
    # In case all primary variables are NA, take max of secondary indicators
    # (currently only used for emerging food)
    mutate(
      underlying = if(length(which(underlying_indicators$secondary == T)) > 0) {
        case_when(
          is.na(underlying) ~
            rowMaxs(as.matrix(select(., any_of(underlying_indicators$indicator_slug[which(underlying_indicators$secondary)])))),
          TRUE ~ underlying)
      } else { underlying },
    ...
    mutate(
      emerging =
        rowMaxs(
          as.matrix(
            select(.,
              any_of(emerging_indicators$indicator_slug[which(!emerging_indicators$secondary)]))),
          na.rm = T) %>%
        { case_when(is.infinite(.) ~ NA_real_, TRUE ~ .) }) %>%
    ...
    mutate(
      emerging = if(length(which(emerging_indicators$secondary == T)) > 0) {
        case_when(
          is.na(emerging) ~
            rowMaxs(as.matrix(select(., any_of(emerging_indicators$indicator_slug[which(emerging_indicators$secondary == T)])))),
          TRUE ~ emerging)
      } else { emerging },
    ...
    mutate(
      underlying_reliability = 
        rowSums(is.na(select(., any_of(underlying_indicators$indicator_slug))), na.rm = T)
          / nrow(underlying_indicators),
      emerging_reliability = 
        rowSums(is.na(select(., any_of(emerging_indicators$indicator_slug))), na.rm = T) 
          / nrow(emerging_indicators)) %>%
...
add_secondary_columns <- function(data, as_of = as_of) {
  ...
  mutate(
    ...
    `Risk Label` = case_when(
      `Data Level` == "Dimension Value" & (Outlook == 'Emerging' | Outlook == 'Underlying') ~
        assign_ternary_labels(Value, 10, 7, 0),
      `Data Level` == "Dimension Value" & Outlook == 'Overall' ~
        assign_ternary_labels(Value, 7, 5, 0),
...
# (raw and normalized) in the dashboard data
last_changed <- indicators_list %>%
  ...
  apply(1, function(indicator) {
    ...
    if (str_detect(dir, ".csv")) {
      ...
      continuity_date <- read_most_recent(
                           paste_path(path, "continuity"), FUN = paste, as_of = as_of, return_date = T)$date
      forecast_date <- read_most_recent(
                          paste_path(path, "forecast"), FUN = paste, as_of = as_of, return_date = T)$date
      max_date <- min(continuity_date, forecast_date)
    } else if (dir == "inform-severity") {
      max_date <- read_most_recent(
                    path, FUN = paste, as_of = as_of, return_date = T, date_format = "%Y%m%d")$date
...
countFlagChanges <- function(data, early = Sys.Date() - 1 , late = Sys.Date()) {
  ...
  prior <- subset(data, Date <= early) %>%
    slice_tail()
  if(sum(select(prior, -Value, -Date, -Record_ID) != select(current, -Value, -Date, -Record_ID)) > 0) {
    warning("Rows do not match between comparison dates.")
  }
  current <- current %>%
    rename(Count = Value)
  prior <- prior %>%
...
quick_scan <- function(
  ...
  source_file = 'production/crm-dashboard-prod.csv') {
    ...
    # for (country in countries) {
        ...
        dims_high <- dims %>% subset(Value >= threshold_high) %>% pull(Dimension)
        dims_med <- dims %>% subset(Value >= threshold_medium & Value < threshold_high) %>% pull(Dimension)
