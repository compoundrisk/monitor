...
| IMF Unemployment | `hosted-data/imf-unemployment/` | IMF World Economic Outlook Database, Unemployment Rate, ISO3 + `.` decimal | Semi-annual (WEO publishes ~April and ~October) | Newest file on disk: **2025-10-01**. An April 2026 WEO edition should exist by now — **likely needs update**. |
...
- **IMF Unemployment** likely has an edition on disk older than a newer one already published by the source — see the Status column in the checklist above, and re-check `hosted-data/imf-unemployment/` for the newest filename before assuming the dashboard reflects the latest edition. (GHSI, GFSI, EIU, FSI, and INFORM Risk, by contrast, are up to date.)
